...
| 6 | 최종 답변 token streaming | **P2(스트레치)**. 기본은 노드 단위 진행 표시(`stream_mode="updates"`) | 02 최소 구조 원칙. 노드 단위로도 처리 상태 연출은 충분 |
...
1. **고객 화면이 주인공이다.** trace 패널은 발표자가 토글로 켜는 보조 화면이다.
...
6. 시연 코스 버튼도 고정 응답이 아니라 실제 `AgentAdapter`를 통과한다.
...
기본(고객 모드)은 채팅 단일 컬럼. 발표자가 `Agent 동작 보기` 토글을 켜면:
...
1. **페르소나 선택** (라디오): 최준혁·조건 명확형 / 정미숙·원금 민감형 / 박서연·연금 탐색형 / 서지우·펀드 초보형.
   변경 시 초기화: messages, thread_id, trace_events, last_candidates, 오프닝·칩 재생성. **기존 thread 재사용 금지.**
2. **고객 컨텍스트 요약** (발표에 필요한 것만): 투자성향 / 투자방식 / 관심분야 / 보유 요약.
3. **시연 코스 버튼** (클릭 → 해당 문구가 사용자 메시지로 입력되어 실제 Agent 실행):
   - ① 조건으로 바로 찾기 → `엔비디아 많이 들어가고 비용 낮은 펀드 찾아줘`
   - ② 후보 상품 비교 → `1번과 3번 비교해줘`
   - ③ 보유종목 겹침 확인 → `나 엔비디아 직접 갖고 있는데 얼마나 겹쳐?`
   - ④ AI 펀드 위험 확인 → `요즘 AI 펀드 어때요?` (정미숙: 성향 초과 차단 안내 / 최준혁: 정상 노출 — 교차 시연)
4. **Agent 동작 보기** 토글 / **대화 초기화** / **펀드 화면으로 돌아가기**
5. **모드 표시**: `LIVE AGENT` 또는 `MOCK TRACE` 배지(패널 상단에도 중복 표시).

## 4. Chat UI 컴포넌트

### 4-1. 오프닝 + 빠른 시작 칩

- 세션 시작 시 `build_opening(profile)`(02 8장) 결과를 첫 assistant 메시지로 표시.
- 칩 3개는 버튼. 명칭은 `빠른 시작`(`추천 질문` 금지 — 01 5-5). 클릭 시 문구가 사용자 메시지로 전송.
- 매 턴 응답 후에는 `AgentTurnResult.chips`(postprocess가 규칙 기반 생성)를 후속 칩 3개로 표시.
...
- Agent 실행 중 노드 단위 진행 문구를 갱신: router → "질문을 이해하고 있어요", search → "관련 펀드를 찾고 있어요", postprocess → "표현과 수치를 점검하고 있어요".
...
표시 원칙:
- 수익률은 고객이 요청했거나 대화 맥락상 필요할 때만 표시하고, 표시 시 반드시 기준기간 병기.
...
도구 호출(`search_funds`, `calc_annual_cost`, `match_overlap`)은 노드가 아니라 해당 노드 아래 들여쓴 이벤트로 표시한다.
...
```text
✅ 금칙 표현 검사 (치환 0건)          ← 치환 발생 시 "⚠ 1건 치환" + 원문/치환문
✅ 수치 대조 (불일치 0건)
✅ 고지 문구 삽입
...
- [ ] 정미숙 ④에서 성향 초과 차단 안내 + 대안 버튼 표시(후보 카드 없음), 최준혁 ④에서는 정상 후보 노출
