...
- 오프닝·빠른 시작 칩은 그래프가 아니라 **세션 시작 시 코드 함수**로 생성한다(7장). LLM 불필요.
...
4. **후속 칩 3개 생성**: LLM 재호출 없이 **규칙 기반**으로 구성 — 현재 action과 State 기준: search 후 → [n번 자세히 보기 / 두 후보 비교하기 / 조건 바꾸기], explain 후 → [관련 상품 살펴보기 / 다른 용어 질문 / ...]. 01 5-5의 이해·찾아보기·비교하기 역할 배분을 따른다.
...
## 8. 오프닝·빠른 시작 칩 — 코드 기반 (LLM 호출 없음)

```python
def build_opening(profile: dict) -> dict:
    """01 6장의 페르소나별 오프닝을 일반화한 템플릿.
    - 문장: 공통 골격(인사→능력 소개→시작 제안)에서 능력 소개 강조점만 프로필 조건으로 분기
      (보유 없음→기초 안내 / search_history에 연금·TDF→용어+비교 / 성향 상위+보유 다수→조건 검색 /
       성향 안정형→위험 설명 강조)
    - 칩 3개: 이해하기·찾아보기·비교하기 역할 1개씩, search_history·holdings 유무로 소재 선택
    - 01 4-4 금지(행동 단정·데이터 복창) 준수는 템플릿이므로 자동 보장
    """
```

시연의 "장면 0 — 같은 에이전트, 다른 첫인사"는 이 함수에 프로필 4개를 넣은 결과를 나란히 보여주는 것으로 구현된다.
...
| 수업 내용 | 사용처 | 채택 이유 |
|---|---|---|
| `init_chat_model("gpt-4o-mini")` (1주차) | 라우터·응답 생성 | 데모 속도·비용, 수업 표준 |
