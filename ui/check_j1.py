...
check("카드 보강: 운용사·수익률·보유종목",
      all(c.get("manager") and c.get("returns_display") and c.get("top_stocks_summary")
          for c in cards))
check("전원 성향 범위 내(카드에 매우높은위험 없음)",
      all(c["risk_grade"] != "매우높은위험" for c in cards))
...
check("chips 3개", len(result["chips"]) == 3)
...
print("④ 정미숙 코스 ④ Live — 차단")
at.radio(key="persona_radio").set_value("P4").run()
check("정미숙 오프닝 상이", "정미숙 고객님" in at.session_state["messages"][0]["content"])
